...
NEWSAPI_KEY = os.getenv("NEWSAPI_KEY")  # set this in secrets
MASSIVE_API_KEY = os.getenv("MASSIVE_API_KEY")  # set this in secrets
...
def fetch_last_trade(ticker: str):
    """
    Use Massive /v2/last/trade/{stocksTicker} to get latest trade price.
    Returns dict or (None, error).
    """
    if not MASSIVE_API_KEY:
        return None, "MASSIVE_API_KEY not set (configure in Streamlit secrets)."

    url = f"{MASSIVE_BASE_URL}/v2/last/trade/{ticker.upper()}"
    params = {"apiKey": MASSIVE_API_KEY}
    try:
        resp = requests.get(url, params=params, timeout=5)
        data = resp.json()
        if resp.status_code != 200:
            return None, f"HTTP {resp.status_code}: {data.get('error', 'Unknown error')}"
        if data.get("status") != "OK":
            return None, data.get("error", "No trade data status OK.")
        results = data.get("results")
        if not results:
            return None, "No last trade results returned."
        price = results.get("p")
        size = results.get("s")
        ts = results.get("t")
        return {"price": price, "size": size, "timestamp": ts}, None
    except Exception as e:
        return None, str(e)


def fetch_recent_daily_bars(ticker: str, days: int = 30):
    """
    Use Massive /v2/aggs/ticker/{ticker}/range/1/day/{from}/{to}
    to get recent OHLC time series.
    Returns (df, error).
    """
    if not MASSIVE_API_KEY:
        return None, "MASSIVE_API_KEY not set (configure in Streamlit secrets)."
    ...
    url = f"{MASSIVE_BASE_URL}/v2/aggs/ticker/{ticker.upper()}/range/1/day/{start}/{end}"
    params = {
        "adjusted": "true",
        "sort": "asc",
        "limit": 5000,
    ...
    try:
        ...
        if resp.status_code != 200:
            return None, f"HTTP {resp.status_code}: {data.get('error', 'Unknown error')}"
        if data.get("status") != "OK":
            return None, data.get("error", "No aggregate data status OK.")
        results = data.get("results", [])
        if not results:
            return None, "No bars returned (maybe ticker or date range issue)."

        # Polygon / Massive style: each result has t, o, h, l, c, v
        rows = []
        for r in results:
            # t is epoch ms; we can leave as ms or convert. Simpler: convert to date only.
            ts_ms = r.get("t")
            if ts_ms is None:
                continue
            # convert ms → date string
        ...
        if not rows:
            return None, "Bars present but no valid timestamps."
...
def fetch_news(keyword="finance", page_size=5):
    """Fetch latest news articles for a given keyword via NewsAPI."""
    if not NEWSAPI_KEY:
        return []
    params = {
        "q": keyword,
...
def compute_stock_sector_impacts(stock_move: float, primary_sector: str) -> pd.DataFrame:
    """
    Illustrative stock → sector sensitivity.

    - Primary sector gets sensitivity 1.0
    - All other sectors get spillover 0.4
    """
    rows = []
    for sec in sectors:
...
def compute_portfolio_exposure(portfolio_df: pd.DataFrame, sector_scores: pd.DataFrame):
    """
    Expects portfolio_df with columns: Sector, Allocation (as percent or units)
          ...
          <div style="display:flex;align-items:center;gap:12px;">
            ...
            <div>
              ...
              <div style="font-size:12px;color:#475569;">
                Live stock snapshot • Sector sensitivity • Portfolio exposure • News & AI research
              </div>
            </div>
          </div>
        </div>
...
with st.sidebar:
    ...
    if not MASSIVE_API_KEY:
        st.warning("Massive API key not configured — live prices will be disabled.")

    if not NEWSAPI_KEY:
        st.info("NewsAPI key not configured — headlines will be disabled.")
...
# ---------- Stock Impact Explorer ----------
with tab_explorer:
    st.subheader("Single-Stock Impact Explorer — Live Snapshot + Sector Sensitivity")
    ...
    with col_top_left:
        ...
        stock_move = st.slider(
            ...
            20,
            0,
            help="Negative = stock down, Positive = stock up (for scenario).",
        )

    with col_top_right:
        st.markdown("### Live Market Snapshot (via Massive.com)")
        if MASSIVE_API_KEY:
            quote, err = fetch_last_trade(stock_name)
            if err:
                st.error(f"Last trade error: {err}")
            elif quote and quote.get("price") is not None:
                st.metric(label=f"{stock_name.upper()} last trade price", value=f"${quote['price']:.2f}")
                st.caption("Data via Massive.com (may be delayed depending on plan).")
            else:
                st.info("No price available for this ticker right now.")
        else:
            st.info("Configure MASSIVE_API_KEY to enable live snapshot.")

        # Recent history chart
        if MASSIVE_API_KEY:
            hist_df, hist_err = fetch_recent_daily_bars(stock_name, days=45)
            if hist_err:
                st.warning(f"History error: {hist_err}")
                ...
                line_chart = (
                    ...
                    .properties(height=220, title=f"{stock_name.upper()} — recent daily closes")
                )
                st.altair_chart(line_chart, use_container_width=True)

    st.markdown("---")
    st.markdown("### Scenario: How this stock move spills into sectors")

    st.caption(
        "This uses a simple, illustrative sensitivity model: the chosen stock has the strongest impact on its "
        "primary sector, and a smaller spillover impact on other sectors."
    )
    ...
    st.caption("Simplified educational model — not a real-world risk engine or investment advice.")

    st.markdown("---")
    st.markdown("### Headlines for this stock")

    if NEWSAPI_KEY:
        news_kw = stock_name.upper()
        articles = fetch_news(news_kw, page_size=5)
        if articles:
            for art in articles:
            ...
            st.write("No recent news found for this keyword.")
    else:
        st.info("Configure NEWSAPI_KEY to see live finance headlines here.")
